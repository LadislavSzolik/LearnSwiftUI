...
/*
 predicate: NSPredicate(format: "category.name == %@", category)

 .onChange(of: category) { value in
   expensesByMonth.nsPredicate = value.isEmpty
   ? nil
   : NSPredicate(format: "category.name == %@", value)
 }
